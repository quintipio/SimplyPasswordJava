...
public class MainViewController implements Initializable {
            ...
            if(t.getButton().equals(MouseButton.PRIMARY)) {
                if(t.getClickCount() >= 1) {
                    try {
                        ...
                        if(selectedMotDepasse != null) {
                            TreeItem<Dossier> dossierTrouve = findDossier(selectedMotDepasse.getDossierPossesseur(),dossierTreeView.getRoot());
                            selectedDossier = (dossierTrouve != null) ? dossierTrouve:dossierTreeView.getRoot();
                            expandTreeView(dossierTreeView.getRoot());
                            MultipleSelectionModel msm = dossierTreeView.getSelectionModel();
     ...
     */
    private TreeItem<Dossier> findDossier(Dossier dossierATrouver, TreeItem<Dossier> dossierDeRecherche) {
        TreeItem<Dossier> trouve = null;
        for (TreeItem<Dossier> tosearch : dossierDeRecherche.getChildren()) {
            if(tosearch.getValue().equals(dossierATrouver)) {
                trouve = tosearch;
                break;
            } else {
